-- StudioOS clean tenant provisioning baseline.
-- Local validation only. No Raven content or production data.
...
CREATE INDEX IF NOT EXISTS idx_reel_assets_tenant_status ON reel_assets(tenant_id, status);
...
CREATE TABLE IF NOT EXISTS growth_evidence_claims (
  ...
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (tenant_id) REFERENCES studioos_tenant_metadata(tenant_id),
  FOREIGN KEY (source_id) REFERENCES growth_evidence_sources(source_id)
);
...
CREATE TABLE IF NOT EXISTS growth_proposals (
  ...
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (tenant_id) REFERENCES studioos_tenant_metadata(tenant_id),
  FOREIGN KEY (hypothesis_id) REFERENCES growth_hypotheses(hypothesis_id)
);
CREATE TABLE IF NOT EXISTS growth_experiments (
  ...
  proposal_id TEXT,
  status TEXT NOT NULL DEFAULT 'DRAFT',
  requires_start_approval INTEGER NOT NULL DEFAULT 1 CHECK (requires_start_approval = 1),
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (tenant_id) REFERENCES studioos_tenant_metadata(tenant_id)
);
...
CREATE TABLE IF NOT EXISTS growth_precision_snapshots (
  ...
  calibration_status TEXT NOT NULL DEFAULT 'insufficient_learning',
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (tenant_id) REFERENCES studioos_tenant_metadata(tenant_id)
);
...
VALUES
  ('raven-oracle', 'raven-oracle', 'raven-guild', 'raven', 'jp', 'JP', 'ja-JP', 1, 'fixture', NULL),
  ('luna-oracle', 'luna-oracle', 'raven-guild', 'luna', 'jp', 'JP', 'ja-JP', 1, 'preview', 'luna-starwind');
