...
export function OpeningCampaignBanner({ config }: { config: TenantOpeningCampaignConfig }) {
  const track = (eventName: string) => { const eventKey = `${config.campaignId}:${eventName}:${window.location.pathname}`; void fetch("/api/opening-campaign/events", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ campaignId: config.campaignId, eventName, eventKey, payload: { page_path: window.location.pathname } }) }); };
