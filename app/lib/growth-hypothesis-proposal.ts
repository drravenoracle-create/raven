...
export class GrowthHypothesisProposalRepository {
  ...
  async updateProposal(tenantId: string, proposalId: string, patch: Partial<Pick<GrowthProposal, "status" | "approvalStatus">>) { const current = await this.getProposal(tenantId, proposalId); if (!current) return null; await this.db.prepare("UPDATE growth_proposals SET status = ?, approval_status = ?, updated_at = CURRENT_TIMESTAMP WHERE tenant_id = ? AND id = ?").bind(patch.status || current.status, patch.approvalStatus || current.approvalStatus, tenantId, proposalId).run(); return this.getProposal(tenantId, proposalId); }
}
