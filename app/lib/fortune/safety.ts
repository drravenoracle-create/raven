...
const highRiskPatterns: Array<{ category: NonNullable<SafetyCheck["category"]>; patterns: RegExp[] }> = [
  ...
  { category: "violence", patterns: [/殴ら/, /暴力/, /DV/, /脅迫/, /逃げたい/, /殺され/] },
  ...
  for (const item of highRiskPatterns) {
    if (item.patterns.some((pattern) => pattern.test(normalized))) {
      return {
        ...
        message:
          "この相談は、占いだけで判断しないほうがよい内容を含んでいます。Raven Oracleとして断定は避けます。まず安全を確保し、医療・法律・お金・身の危険に関わることは専門家や信頼できる人に相談してください。今日できる一歩は、ひとりで抱えず、状況を事実として短く書き出して誰かに共有することです。",
      };
    }
  }
