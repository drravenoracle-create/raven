...
function buildDraft(idea: Idea, tenantId: string) {
  ...
  const structure = isThreeChoice ? { openingHook: hook, choicePrompt: "A・B・Cから、気になる選択肢を選んでください。", resultA: "Aの結果を記入", resultB: "Bの結果を記入", resultC: "Cの結果を記入", closingCta: character.threeChoiceCta, timeline: [{ start: 0, end: 2, label: "Hook" }, { start: 2, end: 5, label: "Choice" }, { start: 5, end: 9, label: "Result A" }, { start: 9, end: 13, label: "Result B" }, { start: 13, end: 17, label: "Result C" }, { start: 17, end: 20, label: "CTA" }] } : { openingHook: hook, body: "鑑定本文を記入", closingCta: character.defaultCta };
