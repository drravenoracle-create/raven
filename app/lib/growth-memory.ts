...
export class GrowthMemoryRepository {
  ...
  async createFromExperiment(tenant: string, experimentId: string, actor = "admin") {
    ...
    await this.db.prepare(`INSERT INTO growth_knowledge_items (id, tenant_id, item_type, title, content, source, status, proposal_id, experiment_id, hypothesis_id, action_type, action_signature, result_status, success_level, predicted_outcome, predicted_value, actual_value, prediction_error, absolute_error, relative_error, direction_predicted, direction_actual, direction_correct, confidence_at_proposal, market, country, locale, reusable, suppressed, updated_at) VALUES (?, ?, 'experiment_learning', ?, ?, 'experiment_result', 'active', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, CURRENT_TIMESTAMP)`).bind(id, t, title, content, proposalId, clean(experiment.experiment_id, 160), clean(experiment.hypothesis_id, 160) || null, clean(experiment.target_type, 80) || null, sig, resultStatus, level, clean(experiment.target_value, 300) || null, predicted, actual, error, abs, relative, predictedDirection, actualDirection, correct, proposal?.confidence ?? num(experiment.confidence_score), proposal?.market || experiment.market || null, proposal?.country || experiment.country || null, proposal?.locale || experiment.locale || null).run();
