...
export async function saveSnsAccount(input: { platform: string; accountId: string; displayName: string; scopes: string[]; accessToken: string; refreshToken?: string; expiresIn?: number }) {
  const row = await (env as unknown as { DB: D1Database }).DB.prepare("SELECT id FROM sns_platform_accounts WHERE tenant_id = ? AND platform = ? LIMIT 1").bind(SNS_TENANT_ID, input.platform).first();
