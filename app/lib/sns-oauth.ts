...
export async function saveSnsAccount(input: { tenantId?: string; platform: string; accountId: string; displayName: string; scopes: string[]; accessToken: string; refreshToken?: string; expiresIn?: number }) {
  const tenantId = input.tenantId || SNS_TENANT_ID;
  resolvePilotSnsConfig(tenantId);
  const row = await env.DB.prepare("SELECT id FROM sns_platform_accounts WHERE tenant_id = ? AND platform = ? LIMIT 1").bind(tenantId, input.platform).first();
