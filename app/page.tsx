        ...
        ...current,
        {
          ...
          text:
            reading.score > 0
              ...
              : "OpenAIはまだ未設定です。鑑定対象の文章を貼ると、ローカル判定で補助できます。",
          at: formatSeconds(Math.max(0, remaining - 15)),
        },
      ]);
      setChatStatus("idle");
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="grid gap-3 rounded border border-[#d8d1c4] bg-[#fffaf1] p-4 text-sm text-[#4c574f] sm:grid-cols-3">
            ...
            <div>
              ...
              <p className="mt-1">OpenAI API / persona: raven-oracle</p>
            </div>
            <div>
          ...
          <div className="grid gap-4 xl:grid-cols-[1fr_0.78fr]">
            ...
            <aside className="rounded border border-[#cfd8cb] bg-[#eef4ec] p-4">
              ...
              <button
                ...
                onClick={runOpenAIReading}
                disabled={!text.trim() || apiStatus === "loading"}
              >
                {apiStatus === "loading" ? "OpenAIで鑑定中..." : "OpenAI鑑定を実行"}
              </button>
              {apiError ? (
              ...
              {openAIReading ? (
                <div className="mt-3 rounded border border-[#b8c4b2] bg-[#fbfff7] p-3">
                  <p className="text-sm font-semibold text-[#526954]">OpenAI鑑定結果</p>
                  <p className="mt-2 whitespace-pre-wrap leading-7 text-[#303a33]">{openAIReading}</p>
                </div>
              ) : null}
