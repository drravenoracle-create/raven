...
export async function GET(request: Request) {
  ...
  const [eventCounts, uniqueVisitors, topPages, referrers, daily, blogViews, externalMetrics, externalConnectors] = await Promise.all([
    ...
    env.DB.prepare("SELECT COUNT(*) AS count FROM analytics_events WHERE tenant_id = ? AND event_name = 'page_view' AND page_path LIKE '/blog/%' AND page_path != '/blog/' AND datetime(created_at) >= datetime(?)").bind(TENANT_ID, since).first<{ count: number }>(),
  ...
  return Response.json(
    {
      ...
      blogViews: Number(blogViews?.count || 0),
