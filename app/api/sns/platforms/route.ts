...
export async function GET(request: Request) {
  ...
  const configured = new Map((settings.results || []).map((row: Record<string, unknown>) => [row.platform, row]));
  const flags = Object.fromEntries((settings.results || []).map((row: Record<string, unknown>) => [String(row.platform), Boolean(row.enabled)]));
