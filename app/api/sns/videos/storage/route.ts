...
const ALLOWED_SOURCE_HOSTS = [new URL(RAVEN_TENANT_CONFIG.publicUrl).hostname, "raven-oracle.dr-ravenoracle.workers.dev"];
...
export async function POST(request: Request) {
  ...
  if (sourceUrl.protocol !== "https:" || !ALLOWED_SOURCE_HOSTS.includes(sourceUrl.hostname)) return Response.json({ error: "source_url host is not allowed." }, { status: 400 });
